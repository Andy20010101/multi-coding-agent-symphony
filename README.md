...
Current tagged release: `v46`, publishing the read-only Workbench Supervisor Dashboard baseline at `/workbench/supervisor/` on top of the v45 backend entrypoint decomposition, the v44.3/v44.4 supervisor read-model and dashboard prototype chain, the v41-v43 controlled runner and supervisor stabilization work, and the v34-v40 App Core chain. The v46 tag does not add Workbench execution, daemon control, provider CLI invocation, or release automation.

Current app shape after v46: the default daily entry is still the local Web Workbench served by `symphony console`. The repo also has the v37 Tauri Desktop Shell boundary under `desktop/shell/src-tauri/`, but that shell is not yet the everyday Mac App entry. The next planned mainline is v47 Mac App Shell Activation, which should make the existing Tauri shell the app-facing entry for local supervision without adding command execution.
...
Open `http://127.0.0.1:8765/workbench/`. Until v47 lands, this browser route is still the default operator path. The daily path is active goal, next action, prompt handoff, event registration, review/revision, main verification, and closeout. The Workbench path follows the current goal/runbook spine:
...
Latest completed mainline release: `v46`. Current released repository tag: `v46`. The `v8` tag remains the stable installer baseline, and `v7` remains available for historical installs. v20-v28 complete the Workbench v1 goal operation chain through release closeout; v29-v32 extend that chain with controlled implementation, adoption, main verification, release baseline/checklist, evidence drafts, and next-version handoff; v33-v37 add the local runtime, action registry, job queue/run control, artifact/evidence index, and Desktop Shell read-only app surfaces; v38-v40 complete the Provider Hub, backup/diagnostics/migration workspace, personal workflow router, app-core release manager, and native UX handoff contracts; v41-v43 cover the controlled provider-runner backend work and supervisor runtime/stabilization evidence; v44.3 and v44.4 establish the supervisor app read model and prototype dashboard; v45 decomposes backend entrypoints; v46 marks the read-only Workbench Supervisor Dashboard baseline.

Current v46 release does not include Autopilot, generic Workbench execution, browser terminal, artifact download, open local file, arbitrary path preview, model invocation, live job runner, persistent job execution, Workbench command execution, daemon start/stop/restart, child dispatch, provider CLI invocation, real CLI invocation, goal event registration, automatic merge, automatic tag, push, publish, GitHub Release creation, release closeout automation, or release-ready inference. Browser views do not execute CLI commands shown as text. The Supervisor Dashboard, Desktop Shell, Action Registry preview, Job Console, and artifact/evidence views are contract/display surfaces and do not run actions or jobs.

Next planned mainline: v47 Mac App Shell Activation. v47 should reuse the existing Tauri shell and `/workbench/desktop/` route to provide an app home surface for current project, sidecar state, backend health, active goal, next action, supervisor summary, and read-only boundaries. It should make backend unavailable, sidecar missing, project missing, active goal missing, stale snapshot, and route failure visible as app states. It should not add a generic shell runner, browser terminal, provider CLI, daemon control, child dispatch, goal event registration, git push/tag/publish, GitHub Release, signed distribution, notarization, or auto-update.
...
The installer clones or updates the `v8` release under `~/.local/share/mcas`, writes `~/.local/bin/symphony` and `~/.local/bin/mcas` shims, installs dependencies with `pnpm install --frozen-lockfile`, and verifies the install with `symphony doctor`. The `v8` tag remains the stable installer baseline; set `MCAS_INSTALL_REF=v46` when you intentionally want the latest tagged release. Set `MCAS_INSTALL_REF=v7` only when you need the historical v7 CLI; `MCAS_INSTALL_DIR`, `MCAS_BIN_DIR`, `MCAS_REPO_SLUG`, and `MCAS_REPO_URL` override the other defaults.
...
The v37 Tauri shell remains available as a native-shell boundary, but the browser Workbench is still the daily path in v46. v47 is planned to activate the Tauri shell as the app-facing entry while keeping the renderer display-only.
...
v18 extends the same console with `GET /api/goals/latest/events` and `GET /api/goals/<goal-id>/events`. Workbench adds Goal Events Timeline and Evidence Matrix panels that display `goal-event-log.v1` and `goal-progress-ledger.v1` fields only. v20-v32 make the goal/runbook flow the Workbench daily path from active goal through release closeout and next-version handoff. v33 adds read-only runtime health/project/snapshot state. v34 adds the read-only Action Registry Panel backed by `/api/actions/manifest`, `/api/actions/availability`, and `/api/actions/preview`. v35 adds the read-only Job Console backed by `/api/jobs`, `/api/jobs/create`, `/api/jobs/timeline`, and `/api/jobs/control`. v36 adds artifact/evidence index, safe preview search, evidence timeline, release bundle, and evidence diagnostics views. v37 adds the Desktop Shell route backed by runtime, project, goal, job, artifact, evidence, and release-bundle contracts. v38 adds the read-only Provider Hub and lane previews. v39 adds backup, diagnostics, migration, and restore validation previews. v40 adds inbox capture, workflow router, goal draft handoff, app-core release manager, and native UX handoff contracts. v41-v43 add controlled provider-runner backend evidence and supervisor runtime/stabilization records. v44.3 adds the read-only supervisor app read model and supervisor API/CLI projection, v44.4 adds the read-only dashboard prototype, v45 splits backend entrypoints, and v46 marks the read-only Workbench Supervisor Dashboard baseline. v47 is planned to activate the existing Tauri shell as the Mac App-facing entry before more dashboard expansion. Evidence refs remain references; the browser and renderer do not download, open, preview arbitrary paths, run actions/jobs, run provider CLIs, control daemons, automate release closeout, or read evidence documents to decide whether a task is approved, main-verified, or release-ready.

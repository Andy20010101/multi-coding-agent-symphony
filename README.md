...
Current tagged release: `v37`, publishing the Desktop Shell MVP on top of the v36 Artifact/Evidence Index Workspace, the v35 Job Queue + Run Control Workspace, the v34 Action Registry Workspace, the v33 local runtime foundation, and the v20-v32 Workbench release-manager chain.
...
Latest completed mainline release: `v37`. Current released repository tag: `v37`. The `v8` tag remains the stable installer baseline, and `v7` remains available for historical installs. v20-v28 complete the Workbench v1 goal operation chain through release closeout; v29-v32 extend that chain with controlled implementation, adoption, main verification, release baseline/checklist, evidence drafts, and next-version handoff; v33 adds the local runtime foundation with health, project registry, current project resolution, and app-state snapshot contracts; v34 adds the read-only Action Registry Workspace with manifest, availability, preview, Workbench panel binding, and migration guidance for later App surfaces; v35 adds the read-only Job Queue + Run Control Workspace with job model, controlled action-to-job creation preview, timeline/log stream contract, pause/cancel/resume/recover transition contract, and Workbench Job Console binding; v36 adds the read-only Artifact/Evidence Index Workspace with derived artifact index, safe preview search, evidence timeline, release bundle, and evidence diagnostics; v37 adds the Desktop Shell MVP with a Tauri host boundary, contract-backed sidecar health, project/goal/next-action, job/run state, artifact preview, evidence readiness, and packaging smoke evidence.

Current v37 release does not include Autopilot, generic Workbench execution, browser terminal, artifact download, open local file, arbitrary path preview, model invocation, live job runner, persistent job execution, automatic merge, automatic tag, publish, or release-ready inference. Browser views do not execute CLI commands shown as text. Desktop Shell, Action Registry preview, Job Console, and artifact/evidence views are contract/display surfaces and do not run actions or jobs.
...
The installer clones or updates the `v8` release under `~/.local/share/mcas`, writes `~/.local/bin/symphony` and `~/.local/bin/mcas` shims, installs dependencies with `pnpm install --frozen-lockfile`, and verifies the install with `symphony doctor`. The `v8` tag remains the stable installer baseline; set `MCAS_INSTALL_REF=v37` when you intentionally want the latest tagged release. Set `MCAS_INSTALL_REF=v7` only when you need the historical v7 CLI; `MCAS_INSTALL_DIR`, `MCAS_BIN_DIR`, `MCAS_REPO_SLUG`, and `MCAS_REPO_URL` override the other defaults.

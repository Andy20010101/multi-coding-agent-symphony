...
Current tagged release: `v59`, publishing the Release Publication Evidence and Next Start Audit chain after v58 release closeout handoff, v57 review gate Workbench surface, v56 thread continuation pack, v55 provider-run recovery handoff, v54 Codex provider execution preview, v53 child dispatch preview, and v52 System Golden Path. The v59 tag and GitHub Release are complete. There is no `v60` tag or GitHub Release yet.

Current main after v59 starts the v60 Stable Personal Workbench Release. v60 is consolidation work for the local Workbench baseline: documentation synchronization, stable baseline acceptance contracts, Workbench findability, acceptance evidence, closeout, and manual release prep. It does not add a generic terminal, renderer-side command execution, unsupported provider support, raw transcript exposure, frontend local session reads, git/tag/publish automation, GitHub Release automation, public distribution, notarization, auto-update, automatic worktree creation, or automatic next-version goal creation.
...
Open `http://127.0.0.1:8765/workbench/desktop/` for the Project Launcher and App Home path, or `http://127.0.0.1:8765/workbench/` for the existing Workbench panels. The daily path now uses the v52-v59 Workbench baseline:

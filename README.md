...
Current tagged release: `v51`, publishing Result Intake Evidence Escrow after the v50 Supervisor Event Preview / Confirm path, the v49 Context Advisory work, and the v48 Project Launcher / App Home path. The v51 tag and GitHub Release are complete. There is no `v52` tag or GitHub Release yet.

Current main after v51 includes v52 `systemGoldenPath.v1` contracts, fixtures, tests, and backend projection into the supervisor app read model. v52 is read-model, Workbench visibility, and acceptance work between the v51 release and later provider execution. It does not add provider execution, child dispatch, new thread product capability, transcript compaction, a generic terminal, or release automation.
...
Open `http://127.0.0.1:8765/workbench/desktop/` for the App Home path, or `http://127.0.0.1:8765/workbench/` for the existing Workbench panels. The daily path now follows the v52 System Golden Path:
...
- v52 System Golden Path adds `systemGoldenPath.v1` as a read-model projection over Project Launcher, App Home, Supervisor, Context Advisory, Result Intake, Event Preview / Confirm, Review / Gate, and Closeout state. Current v52 work is not a release and does not add provider execution, child dispatch, new thread product capability, transcript compaction, generic terminal, or release automation.
...
Latest completed mainline release: `v51`. Current released repository tag: `v51`. The `v8` tag remains the stable installer baseline, and `v7` remains available for historical installs. v20-v28 complete the Workbench v1 goal operation chain through release closeout; v29-v32 extend that chain with controlled implementation, adoption, main verification, release baseline/checklist, evidence drafts, and next-version handoff; v33-v40 add local runtime, app core, provider hub, backup/diagnostics/migration, workflow router, and read-only app surfaces; v41-v46 cover controlled provider-runner backend evidence, supervisor runtime/stabilization, supervisor app read model, dashboard prototype, backend entrypoint decomposition, and the read-only Workbench Supervisor Dashboard baseline; v47-v51 move the daily app path through Mac App Home, Project Launcher, Context Advisory, Event Preview / Confirm, and Result Intake Evidence Escrow. Current main is v52 work with `systemGoldenPath.v1` contracts and backend projection; `v52` is not tagged or released.

Current v51 release and v52 mainline work do not include Autopilot, generic Workbench execution, browser terminal, artifact download, open local file, arbitrary path preview, model invocation, live job runner, persistent job execution, Workbench command execution, daemon start/stop/restart, child dispatch, provider CLI invocation, real CLI invocation outside existing controlled preview/confirm paths, new thread product capability, transcript compaction, automatic merge, automatic tag, push, publish, GitHub Release creation, release closeout automation, or release-ready inference. Browser views do not execute CLI commands shown as text. The Project Launcher, App Home, Supervisor Dashboard, Desktop Shell, Action Registry preview, Job Console, Provider Hub, Result Intake, Event Preview / Confirm, and artifact/evidence views are contract/display or controlled preview/confirm surfaces, not provider execution surfaces.

Current v52 mainline: System Golden Path. The acceptance path is Project Launcher, App Home, Supervisor, Context Advisory, Result Intake, Event Preview / Confirm, Review / Gate, and Closeout. The operator should be able to see selected project, active goal/task, ready and blocked steps, source contracts, and the next safe action without running a provider. Provider execution belongs to a later scoped version.
...
The installer clones or updates the `v8` release under `~/.local/share/mcas`, writes `~/.local/bin/symphony` and `~/.local/bin/mcas` shims, installs dependencies with `pnpm install --frozen-lockfile`, and verifies the install with `symphony doctor`. The `v8` tag remains the stable installer baseline; set `MCAS_INSTALL_REF=v51` when you intentionally want the latest tagged release. Set `MCAS_INSTALL_REF=v7` only when you need the historical v7 CLI; `MCAS_INSTALL_DIR`, `MCAS_BIN_DIR`, `MCAS_REPO_SLUG`, and `MCAS_REPO_URL` override the other defaults.

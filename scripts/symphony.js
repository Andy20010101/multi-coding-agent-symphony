...
import {
  runSymphonyCli as runSymphonyCliFromModules
} from '../src/symphony/cli/index.js';
import {
...
export async function runSymphonyCli({
  ...
  return await runSymphonyCliFromModules({
    argv,
    stdout,
    stderr,
    runner,
    ...
    mcasRunner,
    legacyCommands: {
      ...createLegacyCommandHandlers(),
      ...(legacyCommands ?? {})
    }
  });
}

function createLegacyCommandHandlers() {
  return {
    runWork: runSymphonyWork,
    runScanProduct: runSymphonyScanProduct,
    runWorkProduct: runSymphonyWorkProduct,
    runContinue: runSymphonyContinue,
    runConsole: runSymphonyConsole,
    runDiagnose: runSymphonyDiagnose,
    runRuntime: runSymphonyRuntime,
    runInbox: runSymphonyInbox,
    runActions: runSymphonyActions,
    runProviders: runSymphonyProviders,
    runAppData: runSymphonyAppData,
    runHandoff: runSymphonyHandoff,
    runStage: runSymphonyStage,
    runNext: runSymphonyNext,
    runEvidence: runSymphonyEvidence,
    runBackup: runSymphonyBackup,
    runDiagnosticsBundle: runSymphonyDiagnosticsBundle,
    runRestoreValidation: runSymphonyRestoreValidation,
    runAdopt: runSymphonyAdopt,
    runNew: runSymphonyNew,
    runIntake: runSymphonyIntake,
    runAgent: runSymphonyAgent,
    buildPromptNewTarget
  };
}
...
async function runSymphonyBackup({ args, stdout }) {
  const options = parseBackupArgs(args);

  if (options.help) {
    stdout.write(backupHelpText());
    return EXIT_CODES.ok;
  }
  ...
  if (options.subcommand === 'export') {
    ...
    if (options.json) {
      writeJson(stdout, backupExport);
      return EXIT_CODES.ok;
    }

    stdout.write(renderAppCoreBackupExportText(backupExport));
    return EXIT_CODES.ok;
  }

  throw new UsageError('backup subcommand must be export');
}
...
function parseBackupArgs(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (value === '--output' || value === '-o') {
      throw new UsageError('backup export is manifest/hash/refs only; redirect stdout if you need a file');
    }
    ...
    throw new UsageError(`unknown backup option: ${value}`);
  }
  ...
  if (isUnsafeActionsContextRef(options.goalId) || (options.taskId !== null && isUnsafeActionsContextRef(options.taskId))) {
    throw new UsageError('backup goal and task values must be safe refs');
  }
...
function backupHelpText() {
  return [
    'Usage: symphony backup export [--goal <goal-id>] [--task <task-id>] [--state-dir <path>] [--json]',
    '',

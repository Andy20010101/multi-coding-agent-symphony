...
Start condition: v68 worker/reviewer/adoption/main-verification loop is merged and has fake end-to-end acceptance evidence.
...
v69 should make the new loop durable. The operator should understand and recover from interrupted provider runs, timeouts, missing artifacts, stale plans, dirty worktrees, provider unavailable states, and verification failures.
...
- record usage/cost/time fields as observed, unavailable, or unknown; never fake token/cost data
- add diagnostics summary that redacts secrets, raw logs, raw provider output, and local session paths
...
- Fixtures for worker timeout, reviewer blocked, missing artifact, stale planHash, dirty worktree, verification failure, adoption failure, provider unavailable, unknown failure.
...
Scope:
- Add recovery preview for retry same provider, handoff to other allowed provider, mark blocked, rerun verification, inspect adoption journal, or request operator decision.
...
- Redact secrets, raw logs, raw output, transcript, session paths.
...
Scope:
- Record recovery evidence for at least timeout, provider unavailable, stale planHash, dirty worktree, missing artifact, verification failure.
...
If recovery creates hidden retries, skips source validation, or retries with another provider without explicit operator preview/confirm, revert recovery confirm. If diagnostics expose raw logs, secrets, transcripts, or local session paths, revert diagnostics PR.

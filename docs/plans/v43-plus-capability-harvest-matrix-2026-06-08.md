...
| B3 | Follow-up messaging to an active child was unavailable for some App threads. | Declare follow-up capability at bind time, keep same-session correction inside the runner-owned app-server session, and reject later-session follow-up with explicit operator recovery. | App thread adapter contract: create, read, declare follow-up support, consume result. | ready-to-plan | The capability now has selftest coverage and live replay coverage for both same-session correction and session-mismatch recovery, so the reusable adapter contract is clear. |
| B4 | A valid result can exist before the active thread reader is healthy. | Consume escrow first, let valid closeout escrow register or retire before thread-read dependence, and treat lossy thread reads as non-mutating wait states when no valid escrow exists. | Result consumer state machine with escrow-first ordering and idempotent registration. | ready-to-plan | This is a durable supervisor rule, not a local workaround, and the repository now has live replay evidence for unreadable-thread closeout paths as well as the earlier notLoaded ordering fix. |
| B5 | Daemon health, manual ticks, waiting-operator, and restart behavior need clear separation. | Keep PTY launch and pid/health file mechanics external, but replay and stabilize quiesce, `waiting-operator`, resume-command, and post-restart completion behavior. | Supervisor health model: `daemon-active`, `waiting-operator`, `manual-tick-recent`, `stopped`, `stale`. | candidate | The health-state contract now has live replay coverage, but the launcher/process mechanics are still temporary-only, so only the state model is ready to harvest. |
| B6 | The system cannot yet classify progress versus stall with enough precision to recover automatically. | Implement the task-2 classifier, add deterministic same-lease replay coverage, and expose the signal through `doctor`, `context`, and heartbeat summary output. | Progress observer and stall classifier for child leases. | ready-to-plan | This is a supervisor primitive that future controlled execution also needs. The contract now has deterministic replay coverage plus live monitor/terminal proof, so it no longer depends on operator memory. |
| B7 | Approval-required actions do not yet surface through one reliable user-visible path. | Normalize notices and operator actions in the external runner around one payload contract and one dedupe helper, expose the same payload through `doctor` and `context`, retire resolved notices out of live state, and keep their history in the audit log. | Operator notification bridge and approval event contract. | candidate | The reusable payload and lifecycle contract now has selftest coverage plus live replay proof for creation, dedupe, and retirement. It remains a candidate because the user-facing transport is still tied to temporary App notice threads and thread-store behavior. |
| B8 | External runner behavior can drift outside repository history. | Capture one reusable runner snapshot pair that records script digest, selftest status, launcher command, a bound doctor payload from the same invocation, and the fix list carried after the last release, then cite that same pair from later evidence. | Runner provenance snapshot contract for out-of-repo dependencies. | candidate | The provenance contract now has replayed reuse discipline instead of a one-off capture, but the implementation still depends on the temporary runner path and on in-repo artifact writes during capture, so it remains a candidate rather than a project-internal module plan. |

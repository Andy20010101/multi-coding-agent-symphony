...
| --- | --- | --- | --- | --- | --- |
| B1 | Child thread bypassed the single state writer and directly appended `release.ready-declared`. | Guard against child-written final release events in the external runner and keep release-prep result-only. | Goal supervisor event registrar with explicit write authority rules. | ready-to-plan | This is not a daemon detail. It is a core contract for any future in-project supervisor. |

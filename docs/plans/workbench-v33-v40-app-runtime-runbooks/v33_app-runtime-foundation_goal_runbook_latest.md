# v33 Plan + /goal Runbook: App Runtime Foundation
...
```text
/goal
执行 v33 Task 0：为 `v33-app-runtime-foundation` 写/核对 plan/runbook 和 execution prompts，并用 latest goal init / goal-status 流程注册该 goal。

目标：
- 写入或确认 plan doc：docs/plans/v33-app-runtime-foundation-plan-2026-06-02.md
- 写入或确认 execution prompt doc：docs/plans/v33-app-runtime-foundation-execution-prompts-2026-06-02.md
- 写入或确认 controlled goal-runbook fixture：fixtures/contracts/goal-runbook.v33-app-runtime-foundation.v1.json
- Goal id：v33-app-runtime-foundation
- Baseline：v32 Release Manager Workspace v2
- 版本目标：App Runtime Foundation
- 任务列表必须包含 task-1 到 task-5。
- Workbench 主线必须使用 latest goal/runbook/next-action 命令面。
...
- v34 Action Registry handoff 边界

禁止：
- 不实现产品代码。
- 不创建 Desktop Shell、Action Registry、Job Queue、Provider Hub、Backup/Restore 或 Personal Workflow Router。
- 不新增 permission system、generic shell runner、browser terminal、command DSL、goal framework 或 artifact framework。
- 不登记 task 完成事件。
- 不宣称 release ready。
...
Review evidence: `docs/plans/v33-task-1-review-evidence-2026-06-02.md`
Main verification evidence: `docs/plans/v33-task-1-main-verification-evidence-2026-06-02.md`

## User-visible value

用户能确认本地 app runtime 已启动，并看到 runtime/kernel 版本、进程状态和只读边界。

## Implementation scope

增加 local sidecar skeleton 或等价的 read-only runtime service entrypoint；health API/command 返回 runtime version、kernel version/source、process id、cwd/repo path、read-only mode、startup time、known blockers。

## Acceptance

- health API/command 返回稳定 JSON。
- health response 明确 `read_only: true` 或等价字段。
- health response 记录 runtime/kernel version、cwd/repo path、startup time、known blockers。
- health path 不写 repo、不改 git、不执行 worker/reviewer/main verification/release、不调用模型。
- 有 contract fixture 和自动化测试。
- Workbench 主线仍然使用 latest goal/runbook/next-action 命令面。
...
pnpm --silent symphony goal-status --goal v33-app-runtime-foundation --json
pnpm --silent symphony goal next --goal v33-app-runtime-foundation --json
git checkout -b v33-task-1-local-sidecar-health-api
```
...
- 不把 v8 scan/do/review/verify/status/continue/artifacts 作为 Workbench 顶层模型。
...
- Boundary notes
- Suggested worker evidence path: docs/plans/v33-task-1-worker-evidence-2026-06-02.md
```
...
```text
/goal
为 v33 task-1 写 worker evidence 文档。
...
- Task id: task-1
- Branch: v33-task-1-local-sidecar-health-api
- User-visible value: 用户能确认本地 app runtime 已启动，并看到 runtime/kernel 版本、进程状态和只读边界。
...
- Boundary notes:
  - health path read-only
  - no action execution
  - no job queue
  - no model invocation
  - no git write/release write
...
git status -sb
git add .
git commit -m "Implement v33 task-1: Local sidecar health API"

pnpm --silent symphony goal update \
  --goal v33-app-runtime-foundation \
  --task task-1 \
  --event worker.evidence-recorded \
  --actor codex-v33-task-1-worker \
  --evidence-ref docs/plans/v33-task-1-worker-evidence-2026-06-02.md \
  --dry-run --json
...
Branch: `v33-task-2-project-registry-resolver`
...
Main verification evidence: `docs/plans/v33-task-2-main-verification-evidence-2026-06-02.md`

## User-visible value

app 能列出注册项目，并从 cwd/repo path 解析当前项目。

## Implementation scope

增加 project registry read model 和 current project resolver。registry 字段至少包括 project_id、project_name、repo_path、default_branch、remote_url、last_goal_id、last_run_id、health_status、last_opened_at、pinned。

## Acceptance

- registry 输出为稳定 JSON contract。
- current project resolver 能处理 cwd、显式 repo path、非 git repo、缺失 metadata。
- v33 不创建持久数据库迁移；registry 可以来自 repo-local config、existing metadata 或 fixture。
- resolver 不自动扫描用户全盘。
- resolver 不写 repo、不写 git、不调用模型、不执行 workflow action。
...
pnpm --silent symphony goal-status --goal v33-app-runtime-foundation --json
pnpm --silent symphony goal next --goal v33-app-runtime-foundation --json
git checkout -b v33-task-2-project-registry-resolver
```
...
实现范围：
- 增加 project registry read model。
- 增加 current project resolver，从 cwd 或显式 repo path 解析当前 project。
- registry 字段至少包括 project_id、project_name、repo_path、default_branch、remote_url、last_goal_id、last_run_id、health_status、last_opened_at、pinned。
...
- Files changed
- Tests run with exact results
- Project registry/current project resolver behavior
- Boundary notes
- Suggested worker evidence path: docs/plans/v33-task-2-worker-evidence-2026-06-02.md
```
...
git status -sb
git add .
git commit -m "Implement v33 task-2: Project registry resolver"

pnpm --silent symphony goal update \
  --goal v33-app-runtime-foundation \
  --task task-2 \
  --event worker.evidence-recorded \
  --actor codex-v33-task-2-worker \
  --evidence-ref docs/plans/v33-task-2-worker-evidence-2026-06-02.md \
  --dry-run --json
...
Branch: `v33-task-3-goal-release-state-snapshot`
...
Main verification evidence: `docs/plans/v33-task-3-main-verification-evidence-2026-06-02.md`

## User-visible value

app 能读取当前 project 的 active goal、next action、verification 和 release 状态。

## Implementation scope

增加 app state snapshot API/command。snapshot 至少包含 current_project、runtime_health、active_goal、current_task、next_action、review_status、main_verification_status、release_status、evidence_refs、known_blockers。

## Acceptance

- snapshot 从现有 goal/runbook/event/gate/release 数据读取。
- 缺少 goal 或 release 状态时返回 null/blocker，不推断、不补写。
- snapshot 不执行 goal update/review/gate/closeout confirm。
- snapshot 不创建 action registry、不创建 job queue、不调用模型。
- snapshot contract 有 fixtures/tests。
...
pnpm --silent symphony goal-status --goal v33-app-runtime-foundation --json
pnpm --silent symphony goal next --goal v33-app-runtime-foundation --json
git checkout -b v33-task-3-goal-release-state-snapshot
```
...
- Boundary notes
- Suggested worker evidence path: docs/plans/v33-task-3-worker-evidence-2026-06-02.md
```
...
git status -sb
git add .
git commit -m "Implement v33 task-3: App state snapshot"

pnpm --silent symphony goal update \
  --goal v33-app-runtime-foundation \
  --task task-3 \
  --event worker.evidence-recorded \
  --actor codex-v33-task-3-worker \
  --evidence-ref docs/plans/v33-task-3-worker-evidence-2026-06-02.md \
  --dry-run --json
...
Branch: `v33-task-4-runtime-contract-workbench-surface`
Worker evidence: `docs/plans/v33-task-4-worker-evidence-2026-06-02.md`
Review evidence: `docs/plans/v33-task-4-review-evidence-2026-06-02.md`
Main verification evidence: `docs/plans/v33-task-4-main-verification-evidence-2026-06-02.md`

## User-visible value

UI/CLI 消费同一份 app state schema，用户能在 Workbench 看到 runtime health、current project、active goal、next action 和 release state。

## Implementation scope

定义 app runtime snapshot contract 和 fixtures；Workbench 增加只读 runtime surface 或现有页面区块，显示 health/current project/active goal/next action/release state/known blockers。

## Acceptance

- contract/fixtures 覆盖 healthy、missing project、missing goal、blocked、stale。
- UI 显示 runtime health、current project、active goal、next action、release state、known blockers。
- UI 只调用 app runtime read API，不直接执行 shell 命令。
- loading、empty、blocked、healthy、stale states 都有测试或可验证 fixture。
- UI 不增加执行按钮、不触发 action/job/model/git/release/local file open/artifact download。
...
pnpm --silent symphony goal-status --goal v33-app-runtime-foundation --json
pnpm --silent symphony goal next --goal v33-app-runtime-foundation --json
git checkout -b v33-task-4-runtime-contract-workbench-surface
```
...
- Boundary notes
- Suggested worker evidence path: docs/plans/v33-task-4-worker-evidence-2026-06-02.md
```
...
git status -sb
git add .
git commit -m "Implement v33 task-4: Runtime contract surface"

pnpm --silent symphony goal update \
  --goal v33-app-runtime-foundation \
  --task task-4 \
  --event worker.evidence-recorded \
  --actor codex-v33-task-4-worker \
  --evidence-ref docs/plans/v33-task-4-worker-evidence-2026-06-02.md \
  --dry-run --json
...
Branch: `v33-task-5-runtime-guide-v34-handoff`
...
用户知道如何启动、验证、恢复 v33 runtime，并能直接进入 v34 Action Registry 设计。

## Implementation scope

更新 operator guide 或新增 runtime guide，写清楚启动、health check、project registry、snapshot、known blockers、故障恢复。写 v34 Action Registry handoff，包含 action manifest fields、permission preview fields、available actions API shape、candidate actions、v34 禁止执行边界。

## Acceptance

- guide 有实际命令、状态说明、failure recovery、known blockers。
- v34 handoff 列出 action manifest fields、permission preview fields、available actions API shape、candidate actions。
- v34 handoff 明确 Action Registry 只声明 action availability，不创建 job。
- release evidence 能引用 v33 runtime validation 和 v34 handoff。
- 不自动创建 v34 goal，不实现 v34 代码。
...
pnpm --silent symphony goal-status --goal v33-app-runtime-foundation --json
pnpm --silent symphony goal next --goal v33-app-runtime-foundation --json
git checkout -b v33-task-5-runtime-guide-v34-handoff
```
...
- Files changed
- Tests run with exact results
- Runtime guide path and covered commands
- v34 handoff path and fields
- Boundary notes
- Suggested worker evidence path: docs/plans/v33-task-5-worker-evidence-2026-06-02.md
```
...
git status -sb
git add .
git commit -m "Implement v33 task-5: Runtime guide handoff"

pnpm --silent symphony goal update \
  --goal v33-app-runtime-foundation \
  --task task-5 \
  --event worker.evidence-recorded \
  --actor codex-v33-task-5-worker \
  --evidence-ref docs/plans/v33-task-5-worker-evidence-2026-06-02.md \
  --dry-run --json

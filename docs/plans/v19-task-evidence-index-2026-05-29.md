...
| task-7 | This branch adds README, product contracts, operator guide, this task evidence index, and `docs/plans/v19-release-evidence-2026-05-29.md`; worker evidence should be written separately to `docs/plans/v19-task7-worker-evidence-2026-05-29.md` after validation. No `worker.evidence-recorded` event is registered yet for task-7. | No `reviewer.approved` or `reviewer.needs-revision` event is registered yet for task-7. | No `main.verification-passed` event is registered yet for task-7. | No v19 release gate event is registered for this task. |

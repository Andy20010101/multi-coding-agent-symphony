...
Workbench v1 是日常操作入口。`symphony console` 启动本地服务器后，操作者打开 `/workbench/desktop/` 或 `/workbench/`，按 v52-v60 稳定基线检查 Project Launcher、App Home、Supervisor、Context Advisory、Result Intake、Event Preview / Confirm、Review / Gate、Closeout、Release Publication Evidence 和 Stable Baseline。

当前仓库 release tag 是 `v60`。v60 已完成 annotated tag 和 GitHub Release；`v61` tag/release 不存在。v61 是 Workbench Operator Dry-run Evidence，当前工作是对已发布的 v60 稳定基线做 release-state reconcile、route smoke、operator checklist、recovery drill、closeout 和 v62 handoff，不新增执行面。
...
v60 验收看现有 Workbench 链路是否可解释、可恢复、可发布。v61 在这个基线上记录操作者 dry-run 证据，不看新增执行能力。

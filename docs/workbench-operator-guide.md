...
Workbench v1 是日常操作入口。`symphony console` 启动本地服务器后，操作者打开 `/workbench/desktop/` 或 `/workbench/`，按 v52-v60 稳定基线、v71 本地包路径和 v72 same-day dogfood 记录检查 Project Launcher、App Home、Supervisor、Context Advisory、Result Intake、Event Preview / Confirm、Review / Gate、Closeout、Release Publication Evidence 和 Stable Baseline。

当前仓库 release tag 是 `v72`。`v72^{}` 指向 `cdde20c20931a4e002b184246ad7fd3585fa0979`；GitHub Release 已发布，非 draft、非 prerelease，且没有 assets。当前版本是 `v73 Multi-day Real-use Stabilization`，目标是在个人使用边界内收集至少 3 个连续 Asia/Shanghai 日历日的真实使用证据。

v72 已证明：2026-06-15 同一天 5 个 counted dogfood session、`pnpm desktop:shell:build:local` 本地包构建、macOS `.app` open/quit smoke、`GET /workbench/desktop/` browser fallback、以及缺少 `node_modules` 时用 `pnpm install` 恢复验证路径。
...
v19 增加 Goal Runbook + Next Action Control Center 的实现草稿：`goal-runbook.v1`、`goal-next-action.v1`、`goal-prompt-pack.v1`、`goal-closeout-report.v1`、`symphony goal init`、`symphony goal next`、`symphony goal prompt`、`symphony goal closeout` 和 `symphony next`。v20 到 v32 把 active goal runbook、task queue、prompt handoff、controlled event registration、review/revision、main verification、release closeout、release baseline、release checklist 和 next-version handoff 放到 Workbench 主路径。v33 到 v40 增加 Runtime、Action Registry、Job Console、Artifact/Evidence、Desktop Shell、Provider Hub、backup/diagnostics/migration 和 workflow router 视图；v41 到 v46 增加 supervisor backend/read-model/dashboard 路径；v47 到 v60 增加 App Home、Project Launcher、Context Advisory、Event Preview / Confirm、Result Intake、System Golden Path、child dispatch preview、provider execution preview、provider recovery、thread continuation、review gate、release closeout handoff、release publication evidence 和 stable baseline evidence。summary、runs、handoff、events、capabilities 和 diagnostics 是支撑信息。release-ready 仍需要显式登记 `symphony goal gate --gate release.ready --status declared`，不能从 Workbench 文案、文件名、分支或测试结果推断。

## 当前稳定基线

当前稳定基线来自 v60 的 Workbench 链路、v71 的本地包路径和 v72 的 same-day dogfood 记录。v73 只验证多日真实使用是否稳定，不新增执行能力。

| 区域 | 当前基线 |
| --- | --- |
| Project entry | Project Launcher、current project binding 和 route health 只读展示。 |
| Goal supervision | Active goal、next action、supervisor status 和 route provenance 由后端合同提供。 |
| Context | Context Advisory 和 handoff pack 在需要复制时保持 copy-only。 |
| Result intake | Result Intake preview/confirm 只写 evidence escrow 和 pending result。 |
| Event registration | Event preview 和 planHash confirm 仍由后端 dry-run/confirm 拥有。 |
| Child task planning | Child dispatch preview 只给 copy-only handoff，不创建 child thread 或 worktree。 |
| Provider execution | Codex provider execution preview 和 recovery surface 只在已测试合同内有效。 |
| Review and gates | Review gate、main gate 和 release gate 状态必须来自 explicit event 或合同字段。 |
| Thread handoff | Thread continuation pack 只保留 bounded handoff，不暴露 raw transcript。 |
| Release closeout | v58 handoff、v59 publication evidence 和 v60 stable baseline 只读展示 tag/release 事实和边界。 |
| Release | tag、push、publish、GitHub Release 仍是 controller 手动动作，不进产品代码。 |

禁止项：unsupported provider claims、generic shell/terminal UI、renderer-side command execution、frontend local JSONL/session/provider folder reads、raw transcript/raw model output exposure、direct goal event append、direct task completion、git write/merge/push/tag/publish/GitHub Release automation、public distribution/notarization/auto-update claims、automatic worktree creation、automatic next-version goal creation。
...
`systemGoldenPath.v1` 是 v52 引入的状态合同。它把 Project Launcher、App Home、Supervisor、Context Advisory、Result Intake、Event Preview / Confirm、Review / Gate 和 Closeout 投影成同一条 daily path。它的字段应能显示 step state、source contract、blocked reason、next safe action、route provenance 和 boundary flags。
...
v52 引入时的禁止项保持明确：provider execution、child dispatch、new thread 产品能力、transcript compact、generic terminal、前端读取 session/transcript/provider 文件、git write、merge、push、tag、publish、GitHub Release creation 和 release automation。后续版本只能在已有测试和 fixture 证明的受控合同内展示 provider execution preview、child dispatch preview 或 recovery state，不能扩展成 generic launcher。
...
GET /api/restore/validate -> app-core-restore-validation.v1 -> artifactReadiness.restoreValidation
GET /api/diagnostics/bundle -> app-core-diagnostics-bundle.v1 -> artifactReadiness.diagnosticsBundle
```

桌面页显示 job id、status、queue state、action id、timestamps、blocker/failure、timeline/log counts、run-control transitions、artifact refs/status/missing、safe preview availability、evidence timeline readiness、release bundle state、backup export manifest state、restore validation state 和 diagnostics bundle state。transitions 是只读列表，不是按钮；safe preview 只展示后端 `safe-artifact-preview.v1` 的可用性和 inline safety 字段，不打开本地路径，也不从文件名或扩展名判断安全。backup export 只显示 app core state manifest hash、managed state refs、artifact refs 和 repo content exclusion policy，不复制 repo 内容。restore validation 只显示 manifest integrity、compatible path 和 overwrite default，不执行 restore apply。diagnostics bundle 只显示 sanitized health、recent failure count 和 structured log refs，不复制 raw logs 或 secret values。
...
v39 task-5 增加只读 restore validation：

```text
GET /api/restore/validate
GET /api/restore/validate?goal=<goal-id>&task=<task-id>
pnpm --silent symphony restore validate --goal <goal-id> --task <task-id> --json
```
...
```sh
pnpm --silent symphony providers health --json
```
...
```sh
pnpm --silent symphony providers lanes --json
```

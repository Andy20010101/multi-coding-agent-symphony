...
- v34 `action-manifest.v1` is a declaration surface, not a runner. It lists action ids and maps them to existing controlled contracts, previews, events, and evidence refs. It cannot execute raw shell commands, create jobs, invoke models, read arbitrary files, merge, push, tag, publish, self-approve, or declare release readiness.

...
- v34 `action-manifest.v1` and `action-availability.v1` are declaration and explanation surfaces, not runners. They list action ids, map them to existing controlled contracts, and explain available/unavailable states from explicit goal/task/runbook/next-action data. They cannot execute raw shell commands, create jobs, invoke models, read arbitrary files, merge, push, tag, publish, self-approve, or declare release readiness.

...
v38 task-1 adds `agent-cli-provider.v1` for the Agent CLI Provider Hub MVP. The contract declares exactly two active provider instances: `claude-code-cli` and `codex-cli`. Gemini CLI, Kiro CLI, and DeepSeek are not active provider instances in v38; DeepSeek may appear only as a sanitized backend profile/ref behind an existing local Agent CLI or as future handoff documentation. The contract exposes provider identity, local command name, adapter id, sanitized backend profile status, availability, lanes, gates, health contract refs, and workspace/prompt/output/capability boundaries. It does not execute provider CLIs, dispatch prompts, call models, install providers, open OAuth, expose raw provider settings, expose credential material, or add a generic shell runner.

v38 task-2 adds `agent-cli-provider-health.v1`, exposed through `GET /api/providers/health` and `symphony providers health --json`. The health contract reads only the v38 provider profile and sanitized environment-variable presence for `claude-code-cli` and `codex-cli`. It reports configured/missing provider state, lane availability, blockers, and disabled execution boundaries without executing provider CLIs, dispatching prompts, calling models, reading credential files, exposing credential values, installing providers, opening OAuth, or adding Gemini, Kiro, or DeepSeek as active provider instances.

v38 task-3 adds `agent-cli-capability-profile.v1`, exposed through `GET /api/providers/capabilities` and `symphony providers capabilities --json`. The contract maps action requirements such as `repo.write`, `model.invoke`, `test.run`, and `git.change` to provider gates and tool gates. It anchors the mapping to active goal/task/run/evidence contracts and keeps validation as copy-only command evidence. It does not execute provider CLIs, dispatch prompts, call models, probe provider capabilities, write the repo, merge, push, tag, publish, self-approve, or add Gemini, Kiro, or DeepSeek as active provider instances.

v38 task-4 adds `agent-cli-lane-assignment-preview.v1`, exposed through `GET /api/providers/lane-preview` and `symphony providers lanes --json`. The contract previews worker, reviewer, and main-verifier lane separation from explicit provider, health, capability, goal runbook, next-action, progress, event, and goal-update contracts. Worker/reviewer candidates are limited to `claude-code-cli` and `codex-cli`; the independent review matrix recommends a reviewer provider different from the worker provider, and main-verifier remains an operator-controlled copy-only lane. It does not assign agents, execute provider CLIs, dispatch prompts, call models, approve review, pass main verification, write the repo, merge, push, tag, publish, self-approve, or add Gemini, Kiro, or DeepSeek as active provider instances.
...
v41 task-1 adds `controlled-provider-runner.v1` as the backend contract for provider runner entry and exclusion rules. It keeps active provider ids limited to `claude-code-cli` and `codex-cli`, anchors the runner to explicit goal/task/role/run/evidence context, and records backend-owned command template ids for worker and reviewer lanes. The contract accepts only runner refs such as `goalId`, `taskId`, `role`, `providerId`, `mode`, `promptRef`, `evidenceRef`, and `runId`; it rejects command text, shell fields, argv/args, cwd/path fields, env, credential fields, raw provider settings, and renderer-owned command templates. It defines sanitized output fields and failure layers for schema, provider availability, command execution, timeout, redaction, workspace, and expected checks. It does not implement the task-2 execution adapter, execute provider CLIs, expose raw provider output, add Gemini, Kiro, or DeepSeek as active providers, write git state, approve review, pass main verification, or declare release readiness.
...
v40 task-1 adds `inbox-capture.v1`, exposed through `GET /api/inbox/capture` and `symphony inbox capture --json`. The contract defines the app entry surface for raw user requests, project clues, ideas, and faults before the item is routed into direct answer, skill, automation, Workbench goal, research, or skip flows. Workbench renders the same contract in `InboxCapturePanel`. This path is a read-only capture contract preview: it does not persist capture items, force an active Workbench goal, execute shell commands, call models, open local files, create jobs, write git state, approve review, pass main verification, or declare release readiness.

v40 task-2 adds `workflow-router-categories.v1`, exposed through `GET /api/workflow/router-categories` and rendered in Workbench as `WorkflowRouterCategoriesPanel`. The contract lists six request categories: direct answer, skill, automation, workbench goal, research, and ignore/skip. Each category names request signals, the user path, allowed source contracts, and the next step. The panel shows routing examples and disabled boundary flags. This path does not write route decisions, create goal drafts, create jobs, execute actions, fetch research, invoke models, run shell commands, write git state, self-approve, pass main verification, or declare release readiness.
...
- v33 `local-runtime-health.v1` is read-only runtime status. It reports the local sidecar process, v32 kernel source, and v37 sidecar host lifecycle state, but it cannot execute actions, create jobs, invoke models, write git state, register goal events, or write release state.
...
- v34 `action-manifest.v1`, `action-availability.v1`, and `action-preview.v1` are declaration and explanation surfaces, not runners. They list action ids, map them to existing controlled contracts, explain available/unavailable states from explicit goal/task/runbook/next-action data, and preview action impact plus required confirmations. They cannot execute raw shell commands, create jobs, invoke models, read arbitrary files, merge, push, tag, publish, self-approve, or declare release readiness.
- v35 `job-model.v1`, `job-creation.v1`, `job-timeline-log-stream.v1`, and `job-run-control.v1` are read-only job contract surfaces. They model job identity, controlled action-to-job dry-run creation, timeline/log refs, and pause/cancel/resume/recover semantics. They cannot persist jobs, execute jobs, run shells, invoke models, mutate frontend state, write git/release state, push, tag, publish, self-approve, or declare release readiness.
- v37 `sidecar-host-lifecycle.v1` is the Desktop Shell native host boundary for sidecar attach/launch state. It records attach state, health route, fixed launcher command id, loopback host/port scope, and boundary flags. The native bridge may launch only the known console sidecar command shape; the renderer does not receive a generic shell runner, arbitrary command text, arbitrary path input, model invocation, git write, release write, push, tag, or publish surface.
- v37 `DesktopShellMvpViewModel` is a Workbench renderer projection for `/workbench/desktop/`. It combines existing read-only contract fields for sidecar health, project list, active goal, next action, blocked/review/main verification/release status, job/run state, artifact preview, and evidence readiness. It reads project list state from `project-registry.v1`, sidecar attach/launcher state from `app-state-snapshot.v1.runtime_health.sidecarHost`, and goal status from snapshot/ledger/next-action/event contracts. It does not replace ArtifactStore, goal-event-log, job contracts, project registry, or runtime snapshot. It cannot execute shell commands, call models, open arbitrary local files, create jobs, mutate git, merge, push, tag, publish, approve review, pass main verification, or declare release readiness.
- v38 `agent-cli-provider.v1` is a read-only provider profile contract. It can show active provider ids, display names, local command names, adapter ids, sanitized backend profile refs, `configured`/`missing`/`unknown` status, lane/gate fields, and disabled execution boundaries. It cannot expose API keys, OAuth tokens, credential file contents, raw provider settings, full secret-bearing config, raw shell command lines, provider CLI execution, renderer-side provider invocation, prompt dispatch, model invocation, automatic provider install, OAuth login, task-2 health API behavior, task-3 capability mapping, task-5 Provider Hub panel behavior, git writes, push, tag, publish, self-approval, or release readiness.
- v38 `agent-cli-provider-health.v1` is a read-only provider health contract. It can show sanitized required-env presence, provider health state, lane assignability, and explicit missing-provider blockers for `claude-code-cli` and `codex-cli`. It cannot expose env values, API keys, OAuth tokens, credential files, raw provider settings, provider CLI execution results, renderer-side provider invocation, prompt dispatch, model invocation, generic shell execution, automatic install, OAuth login, task-3 capability mapping, task-5 Provider Hub panel behavior, git writes, push, tag, publish, self-approval, or release readiness.
...
- v41 `controlled-provider-runner.v1` is the backend runner contract for `claude-code-cli` and `codex-cli`. It can show allowed runner input fields, rejected shell/path/secret fields, backend-owned command template ids, sanitized output fields, explicit failure layers, and the boundary that provider CLI execution is allowed only through a backend controlled runner. It cannot accept arbitrary command text, renderer-built commands, arbitrary local paths, env values, credential material, raw provider settings, raw provider output, Gemini, Kiro, DeepSeek, merge, push, tag, publish, self-approval, main verification, or release readiness.
- v52 `systemGoldenPath.v1` is the read-model contract for the current daily path: Project Launcher, App Home, Supervisor, Context Advisory, Result Intake, Event Preview / Confirm, Review / Gate, and Closeout. It can report `ready`, `pending`, `blocked`, `missing`, `stale`, `degraded`, or `manual-required` states with source contracts and next safe action. It cannot execute providers, dispatch children, create threads, compact transcripts, read raw session or transcript files from the frontend, run a terminal, mutate goal/review/main/release gates, write git state, merge, push, tag, publish, create GitHub Releases, or infer readiness from UI render success.
...
- v40 `workflow-router-categories.v1` is a read-only routing category contract. It can show direct answer, skill, automation, workbench goal, research, and ignore/skip routes with request signals, allowed contracts, next steps, examples, and boundary flags. It cannot write router state, create goal drafts, register goals, create jobs, execute actions, fetch research, invoke models, run shell commands, write git state, merge, push, tag, publish, self-approve, pass main verification, or declare release readiness.
...
`action-manifest.v1` declares the Workbench/App action surface that can be rendered or previewed by later tasks. It is a backend contract, not an execution interface.
...
symphony actions manifest --goal <goal-id> --task <task-id> --json
```

The route accepts only optional `goal` and `task` query parameters. The CLI writes only to stdout and rejects output-file flags. Both surfaces return the same contract family. They do not confirm actions, create jobs, append goal events, execute shell commands, call models, open local files, merge, push, tag, publish, approve, verify, or declare release readiness.

## `action-availability.v1`

`action-availability.v1` explains the current state of each declared action for an active goal/task context. It reads existing backend contracts and returns reasons; it does not execute or confirm anything.
...
```json
{
  ...
  "context": {
    ...
    "nextAction": {
      ...
      "role": "worker",
      "phase": "implement",
      "reason": "No explicit worker evidence is recorded for task-2.",
      "blocked": false
    }
  },
  "actions": [{
...
symphony actions availability --goal <goal-id> --task <task-id> --json
```

`state` is `available`, `unavailable`, or `blocked`. `reasons` explain unavailable states from `goal-next-action.v1` and `goal-progress-ledger.v1`. `requiredInputs` names operator-supplied fields such as an evidence ref; those inputs do not make an otherwise valid action unavailable. The route accepts only optional `goal` and `task` query parameters and rejects unsafe route segments. The CLI writes only to stdout. The contract cannot append events, run previews, create jobs, execute shell commands, invoke models, read evidence bodies, merge, push, tag, publish, self-approve, or declare release readiness.

## `action-preview.v1`

`action-preview.v1` shows what a declared action would require before confirmation. It reuses `action-availability.v1`; it does not execute the action or append events.
...
The route accepts only optional `goal`, `task`, and `action` query parameters. `action` filters by declared action id and does not execute that action. The preview reports `capabilities`, `requiredConfirmations`, `impactPreview`, endpoint safety, and the same no-write boundaries as manifest and availability. It rejects prompt, confirm, planHash, command, path, and unsafe action-id inputs. It does not start implementation runs, append goal events, run shell commands, call models, read evidence bodies, merge, push, tag, publish, self-approve, or declare release readiness.

## v35 Job Queue + Run Control contracts

v35 turns the v34 action layer into read-only job contracts that App surfaces can inspect. It does not add a live job runner.
...
`GET /api/jobs` returns `job-model.v1`. The contract names the job id, project/goal/task/action refs, status, queue state, blocker, failure field, source contract refs, and boundary fields. `jobCreationSource` is locked to `action-preview.v1 only`. It rejects unsafe refs and unsupported query parameters.

`GET /api/jobs/create` returns `job-creation.v1`. The contract is a dry-run creation model from a controlled action preview. It carries the source `action-preview.v1`, required source contracts, warnings, blockers, and plan boundary fields. It does not persist a job, confirm a goal event, execute an action, call a model, or write files.

`GET /api/jobs/timeline` returns `job-timeline-log-stream.v1`. The contract defines job timeline event shape, queue-state fields, blocker/failure fields, and log ref shape. With no real job event store, it returns empty `timeline` and `logRefs` arrays rather than inventing state.

`GET /api/jobs/control` returns `job-run-control.v1`. The contract defines exactly four controlled transitions:
...
The control route computes available transitions from explicit backend state only. It does not mutate job state, retry work, start a runner, or write the timeline.

Workbench `projectJobConsole` projects these four route results into the Job Console panel. The panel displays queue state, creation status, warnings/blockers, timeline/log counts, transition table, route health, and safety boundaries. It has no execution handlers, job state setters, browser storage writes, shell calls, model calls, git writes, release writes, push/tag/publish paths, or self-approval paths.

All four job routes remain GET-only. Tests cover allowlist membership, contract validation, non-GET rejection, unsafe query rejection, and static Workbench build markers.
...
`sidecar-host-lifecycle.v1` is a v37 task-2 sub-contract inside `local-runtime-health.v1`. It gives Desktop Shell and native host code one explicit attach/launch status object:
...
`attach.state` is `attached`, `detached`, or `unavailable`. `launcher.state` is `defined` or `unavailable` in backend runtime health. The Tauri host command can return `launch-requested` after it starts the fixed sidecar process.

The allowed launch target is loopback only:

```text
host: 127.0.0.1 or localhost
port: 1024..65535
commandId: symphony.console.sidecar.launch
native command shape: pnpm symphony console --host <loopback> --port <allowed-port>
```

The contract cannot carry arbitrary command text, executable paths, file paths, model options, git writes, release writes, push, tag, publish, or approval fields. The renderer route reads it through the runtime snapshot and does not execute the native command in task-2.
...
`active_goal`, `current_task`, `review_status`, `main_verification_status`, and `release_status` come from managed runbook state, `goal-progress-ledger.v1`, `goal-next-action.v1`, event/gate evidence, and release state. `runtime_health.sidecarHost` comes from `local-runtime-health.v1.sidecarHost`. Missing active goal or release state is reported as `null` with a blocker entry. The snapshot does not infer status from filenames, branch names, commits, prompt text, task titles, copy-only commands, or frontend-only state.
...
v34 now exposes three read-only action contracts:
...
Implemented action preview fields:
...
Implemented action ids for this pass:

```text
goal.worker-evidence.record
goal.review-verdict.record
goal.main-verification-gate.record
goal.release-gate.record
goal.implementation.preview
```

No-job-execution boundary: v34 Action Registry may declare actions, explain availability, show preview impact, list required confirmations, and map actions to existing controlled contracts. It must not create jobs, run actions, execute shell commands, invoke models, write files, write git state, merge, push, tag, publish, download artifacts, open local files, register release readiness, or create the v34 managed goal automatically.
